...
//hs note - disable this to simplify the css and then rebuild... later
...
if(!function_exists('qode_startit_google_fonts_styles')) {
    ...
    function qode_startit_google_fonts_styles() {
        ...
        $default_font_string = 'Nunito:'.$font_weight_str;
...
//hs        wp_enqueue_script( 'jquery-ui-core');
//hs        wp_enqueue_script( 'jquery-ui-tabs');
//hs        wp_enqueue_script( 'jquery-ui-accordion');
//hs        wp_enqueue_script( 'wp-mediaelement');
//hs        wp_enqueue_script( 'jquery-ui-slider');
...
//hs        wp_enqueue_script('qode_startit_modules', QODE_ASSETS_ROOT.'/js/modules.min.js', array('jquery'), false, true);
...
function synthio_scripts() {
  
  $ctime = filemtime( get_template_directory() .'/assets/custom-styles/styles.css');
  $jtime = filemtime(get_template_directory() . '/assets/scripts.js');
  
  wp_enqueue_style('synthio-styles', get_template_directory_uri() .'/assets/custom-styles/styles.css', array(), $ctime);
  wp_enqueue_script('synthio-scripts_bgcol', get_template_directory_uri() . '/assets/js/background-check/background-check.js');  
  wp_enqueue_script('synthio-scripts', get_template_directory_uri() . '/assets/scripts.js', ['jquery', 'jquery-ui-core', 'hoverIntent'], $jtime);
}

add_action( 'wp_enqueue_scripts', 'synthio_scripts' );
...
require get_template_directory() . '/includes/gutenberg-acf.php';


function synthio_resource_filter() {
  
  // hard-coding the destination for now in the interest of speed.
  $current = $_GET["category"];
  $output = "<form class='resource-filter' action='/resources'>
  <label for='resource-filter-category'>Filter By:</label>
  <select id='resource-filter-category' name='category'><option value='all'>All</option>";
  $terms = get_terms( array( 'taxonomy' => 'portfolio-category', 'parent' => 0 ) );
...
add_shortcode( 'resource_filter', 'synthio_resource_filter' );
function synthio_add_query_vars_filter( $vars ) {
  $vars[] = "category";
  return $vars;
}
add_filter( 'query_vars', 'synthio_add_query_vars_filter' );
...
add_shortcode( 'blog_title', 'myshortcode_title' );
